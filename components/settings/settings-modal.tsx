...
const getPageTitle = (pageId: string): string => {
  for (const section of siteConfig.dashboard.settings) {
...
const SettingsModal = () => {
  const { isOpen, close, activePage, setActivePage } = useSettings()
  const { name, email, image, initials } = useUser()
  const sections = siteConfig.dashboard.settings
      ...
      <DialogPortal>
        ...
        <DialogPrimitive.Popup
          ...
          style={
            {
              "--settings-chrome":
                "color-mix(in oklch, var(--background) 95%, var(--foreground))",
            } as React.CSSProperties
          }
        >
          <div
            className="grid h-[min(80vh,680px)] grid-cols-1 sm:grid-cols-[240px_1fr]"
            style={{ background: "var(--settings-chrome)" }}
          >
            <aside className="hidden flex-col sm:flex">
